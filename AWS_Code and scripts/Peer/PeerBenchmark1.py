...
class Peer():
    ...
    def obtain(self, file_name, peer_request_id):
        ...
        try:
            ...
            obtain_time = 0
            total_bytes = 0
            self.service.establish_connection()
            self.get_file_list()
            register = False
            for file in self.file_list:
            ...
            for i in range(total_ops):
                file_name = "1GB_fda1.txt"
                response = self.service.get(file_name)
                peer_id = random.choice(response)
                peer_request_addr, peer_request_port = peer_id.split(':')
                peer_request_socket = \
                ...
                t2 = time.time()
                obtain_time = obtain_time + (t2-t1)
                fileinfo = os.stat("./Download/"+file_name)
                filesize = fileinfo.st_size
                total_bytes += filesize
            self.service.disable_connection()
            print "%s Obtain operations = %s sec" % (total_ops,obtain_time)
            print "Total Bytes transferred %s " % (total_bytes)
            print "Throughput = %s bytes/sec" % (total_bytes/obtain_time)
            #print "per Obtain operation = %s msec" % (((obtain_time)/total_ops)*1000)
        except Exception as e:
